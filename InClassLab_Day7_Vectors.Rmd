...
plot(x_range, sin_wave, type = "l", col = 'red', lwd = 3)
```
...
```{r}
first_nums <- 40:50
next_nums <- seq(from = 51, to = 60, by = 1)
combine <- c(first_nums, next_nums)
combine[7]
combine[5:8]
length(combine)
first_three <- combine[c(1:3)]
last_three <- (c(length(combine) - 2) : length(combine))
new_vector <- combine[c(1:3, 19:21)]
